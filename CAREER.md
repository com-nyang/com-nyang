...
애플리케이션 개발과 인프라/배포 자동화의 연결 지점에 관심이 많으며, 서비스 기능 개발뿐 아니라 AWS, Kubernetes, Terraform, GitHub Actions 기반의 DevOps/IaC 영역까지 함께 다뤄본 경험이 있습니다. 기능을 만드는 것에서 끝내지 않고, 서비스가 어떤 인프라 위에서 배포되고 운영될 수 있는지까지 고려하는 개발자를 지향합니다.

Go 백엔드와 React Native/React 기반 프론트엔드 개발을 함께 수행하며, 모바일 앱, Admin Web, 백엔드 API가 연결된 서비스 기능을 end-to-end로 개발했습니다. 크루 기반 커뮤니티/미션 인증 플랫폼에서 미션 생성, 인증 제출, 자동 승인/수동 검수, 하루 1회 인증 제한, 인증 게시판 권한 제어 등 주요 정책 기능을 API 계약 설계부터 프론트 상태 처리까지 연동했습니다.
...
- AWS 인프라를 콘솔 수작업이 아닌 Terraform 기반 IaC로 구성하는 방식에 관심을 갖고 ECR, VPC, EKS, IRSA 모듈을 설계 및 구성했습니다.
- Kubernetes/EKS 기반 배포 구조를 이해하기 위해 서비스별 Deployment, Service, ServiceAccount, Kustomize base/overlay 구조를 설계했습니다.
- GitHub Actions, image tag 자동 반영 스크립트, migration Job 분리 등 배포와 운영 과정에서 반복되는 작업을 자동화하는 방향으로 개선했습니다.
- 보안 관점에서 Secrets Manager와 IRSA를 활용해 서비스별 권한을 분리하고, 최소 권한 기반의 secret 접근 구조를 설계했습니다.
...
- Go, Gin, Python, Django, Rust, PostgreSQL, Redis, S3
- AWS ECS, CloudFront
...
- DevOps/IaC 관점에서 Terraform과 Kustomize를 활용해 기존 ECS 중심 인프라를 EKS/Kubernetes 기반 구조로 전환하기 위한 ECR, VPC, EKS, IRSA, Gateway API routing, migration Job, image 자동 반영 스크립트 등 운영 배포 기반 구성을 설계 및 구현했습니다.
